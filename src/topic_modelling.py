...
def preprocess_into_tokens(
    trope_description: str, minimum_token_length: int = 4
) -> list[str]:
    """
...
def create_gdsmm_model(
    ...
    for topic in top_topics:
        top_words_and_distributions = sorted(
            mgp.cluster_word_distribution[topic].items(),
            key=lambda k: k[1],
            reverse=True,
        )[:num_words]
        topic_file.write(
            "{}|{}\n".format(
                list(top_topics).index(topic),
                "|".join(pair[0] for pair in top_words_and_distributions),
            )
        )
